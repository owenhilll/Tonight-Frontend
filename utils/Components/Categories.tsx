      ...
      {index == 0 && (
        <Animated.View entering={FadeInRight.duration(500)} exiting={FadeOutLeft.duration(500)}>
          ...
          <TouchableOpacity
            ...
            onPress={handleNext}>
            <FontAwesome6 iconStyle="solid" size={15} color="#BBDEFB" name="arrow-right" />
          </TouchableOpacity>
        </Animated.View>
      )}
      {index == 1 && (
        <Animated.View entering={FadeInRight.duration(500)} exiting={FadeOutLeft.duration(500)}>
          <TouchableOpacity className="absolute left-2 top-2 w-7 text-white" onPress={handlePrev}>
            <FontAwesome6 color="#BBDEFB" size={15} iconStyle="solid" name="arrow-left" />
          </TouchableOpacity>
          <View className="mx-5 flex-row justify-center py-2">
            <CategoryCard handleClick={() => handleClick('Game')}>
              <FontAwesome6 color="#BBDEFB" size={15} iconStyle="solid" name="gamepad" />
              <Text className="text-center text-sm text-white">Games</Text>
            </CategoryCard>
            <CategoryCard handleClick={() => handleClick('outActivity')}>
              <FontAwesome6 color="#BBDEFB" size={15} iconStyle="solid" name="sun" />
              <Text className="text-center text-sm text-white">Outdoors</Text>
            </CategoryCard>
            <CategoryCard handleClick={() => handleClick('Classes')}>
              <FontAwesome6 color="#BBDEFB" size={15} iconStyle="solid" name="school" />
